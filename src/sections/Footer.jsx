      ...
      <div className="relative c-space mx-auto max-w-7xl">
        <div className="flex flex-col gap-8 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-xl">
            <a href="#home" className="mt-2 inline-block text-3xl font-bold leading-none tracking-tight text-white hover:text-aqua transition-colors">
              Randika
            </a>
            <p className="mt-4 text-sm leading-relaxed text-neutral-300 md:text-base">
              Software Engineer at <strong className="text-white">Smart Omega Group</strong>. 
              Engineering scalable web applications, real-time mobility systems, and high-performance digital experiences.
            </p>
          </div>

          <div className="flex flex-col gap-3">
            <p className="text-xs uppercase tracking-[0.2em] text-neutral-400 font-semibold">Quick Links</p>
            <nav className="flex flex-wrap items-center gap-2 text-sm text-neutral-200">
              {quickLinks.map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                  className="rounded-full border border-white/15 bg-white/5 px-3.5 py-1.5 text-xs transition hover:-translate-y-0.5 hover:border-aqua/50 hover:bg-aqua/10 hover:text-white"
                >
                  {link.label}
                </a>
              ))}
        ...
        <div className="mt-8 flex flex-col gap-4 border-t border-white/10 pt-6 md:flex-row md:items-center md:justify-between">
          <p className="text-sm text-neutral-300">
            Copyright &copy; {new Date().getFullYear()} Randika. All rights reserved.
          </p>
